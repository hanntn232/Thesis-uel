...
Original file is located at
    https://colab.research.google.com/drive/1kNBe4w5HkESisLqmym7NXt-CfzoaiAYB
"""

#import libraries to create neural network
import torch.nn as nn
import torchvision.models as models
...
class TSEmbedder(nn.Module):
  ...
  def forward(self, x):
    # apply dropout rate for neural network
...
class TemporalFeatureEncoder(nn.Module):
  ...
  def forward(self, temporal_features):
    #convert features from 2D tensor to 3D tensor
    ...
    w_emb = self.dropout(self.week_embedding(w))
    m_emb = self.dropout(self.month_embedding(m))
    y_emb = self.dropout(self.year_embedding(y))
...
class ImageEncoder(nn.Module):
  def __init__(self, embedding_dim, fine_tune=False):
    ...
    for p in self.cnn.parameters(): 
      #freeze all of the network (or not)
  ...
  def forward(self, x):
    out = self.cnn(x)
    # Because aux_logits = True 
    # So the output of model inception_v3 will be the tuple 
    # This tuple contains both logits and aux_logits. 
    # We need to choose logits
    if torch.is_tensor(out) == False: 
      out = out.logits
    #---
    out = torch.reshape(out,[-1,64,2048]) 
    out = self.dropout(self.fc(out))
    return out

class AdditiveAttention(nn.Module):
  # Bahdanau encoder-decoder attention (Additive attention)
  ...
  def forward(self, encoder_out, decoder_hidden):
    ...
    attention_weight_encoding = ( alpha.unsqueeze(2) * h_j ) # (batch_size, len, encoder_dim)
